...
from lib.models.mymodules.former import Mlp, Scale, DropPath, LayerNorm3d, ClassMamba, ClassAttention
...
class EncoderStage(nn.Module):
    ...
    def forward(self, x):
        ...
        x = window_reverse(windows, self.window_size, B, D, H, W, C)  # (B, D, H, W, C)
        x_skip = rearrange(x, "b d h w c -> b c d h w")
        x_skip = self.skip_conv(x_skip)
        x_down = self.patch_merging(x)  # (B, D//2, H//2, W//2, 2*C)
...
class SlimFormer(nn.Module):
    def __init__(self,
                 ...
                 prune_flags_list=[ [{"prune_ln2_mlp": False, "prune_ln1_token_mixer": False},
                                     {"prune_ln2_mlp": False, "prune_ln1_token_mixer": True}
                                   ],
    ...
    def forward(self, x):
        ...
        x0 = self.patch_embed(x)  # (B, base_channels, D0, H0, W0)
        x0 = self.pos_drop(x0)
        ...
        d3 = self.decoder3(x3, skip3)       # d3: base_channels*4
...
if __name__ == '__main__':
    ...
    flops, params = get_model_complexity_info(model, (1, 96, 96, 96), as_strings=True,
