...
# ClassMamba, ClassRNN 和 ClassAttention 无需修改，因为它们处理的是单纯的 token 序列
# 不依赖于输入数据的维度（2D 或 3D）

###############################################################################
# 需要修改的类：LayerNorm3d -> LayerNorm2d
###############################################################################
class LayerNorm2d(nn.Module):
    """
    对 2D 特征图 (B, C, H, W) 使用 LayerNorm：
...
# DropPath, Scale, Mlp 类不需要修改，因为它们与输入数据的维度无关
###############################################################################
# ClassMamba
###############################################################################
