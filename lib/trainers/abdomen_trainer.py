...
class AbdomenTrainer:
        ...
        if not self.opt["optimize_params"]:
            if self.opt["resume"] is None:
                self.execute_dir = os.path.join(opt["run_dir"],
                                                utils.datestr() + "_" + opt["model_name"] + "_" + opt["dataset_name"])
            else:
    ...
    def training(self):
        ...
        # self.valid_epoch(-1)
        # pre_valid_dsc = self.statistics_dict["valid"]["DSC"]["avg"] / self.statistics_dict["valid"]["count"]
        # print("Pre-training validation DSC: {:.6f}".format(pre_valid_dsc))
        ...
        for epoch in range(self.start_epoch, self.end_epoch):
            ...
            train_loss = self.statistics_dict["train"]["loss"] / self.statistics_dict["train"]["count"]
            train_dsc = self.statistics_dict["train"]["DSC"]["avg"] / self.statistics_dict["train"]["count"]
            ...
            if valid_dsc is not None:
                ...
                print(
                    "epoch:[{:03d}/{:03d}]  lr:{:.6f}  train_loss:{:.6f}  train_dsc:{:.6f}"
                        .format(epoch, self.end_epoch - 1,
                                self.optimizer.param_groups[0]['lr'],
                                train_loss,
                                train_dsc)
            ...
            if not self.opt["optimize_params"]:
                log_str = ("epoch:[{:03d}/{:03d}]  lr:{:.6f}  train_loss:{:.6f}  train_dsc:{:.6f}"
                           .format(epoch, self.end_epoch - 1,
                                   self.optimizer.param_groups[0]['lr'],
                                   train_loss,
                                   train_dsc))
                if valid_dsc is not None:
                    log_str += "  valid_dsc:{:.6f}  best_dsc:{:.6f}".format(valid_dsc, self.best_dice)
                utils.pre_write_txt(log_str, self.log_txt_path)

            if self.opt["optimize_params"] and (epoch + 1) % 5 == 0:
    ...
    def train_epoch(self, epoch):
        ...
        train_loader_tqdm = tqdm(self.train_data_loader, desc=f"Train Epoch {epoch}", total=len(self.train_data_loader))
        for batch_idx, (input_tensor, target) in enumerate(train_loader_tqdm):
            ...
            t8 = time.time()
            self.calculate_metric_and_update_statistcs(output.cpu().float(), target.cpu(), len(target), dice_loss.cpu(),
                                                       mode="train")
            t9 = time.time()

            if (batch_idx + 1) % self.terminal_show_freq == 0:
                current_loss = self.statistics_dict["train"]["loss"] / self.statistics_dict["train"]["count"]
                current_dsc = self.statistics_dict["train"]["DSC"]["avg"] / self.statistics_dict["train"]["count"]
                train_loader_tqdm.set_postfix(lr=self.optimizer.param_groups[0]['lr'],
                                              loss=f"{current_loss:.6f}",
                                              dsc=f"{current_dsc:.6f}")
                if not self.opt["optimize_params"]:
                    utils.pre_write_txt(
                        "epoch:[{:03d}/{:03d}]  step:[{:04d}/{:04d}]  lr:{:.6f}  loss:{:.6f}  dsc:{:.6f}"
                        .format(epoch, self.end_epoch - 1,
    ...
    def valid_epoch(self, epoch):
        ...
        valid_loader_tqdm = tqdm(self.valid_data_loader, desc=f"Valid Epoch {epoch}", total=len(self.valid_data_loader))
        with torch.no_grad():
            for batch_idx, (input_tensor, target) in enumerate(valid_loader_tqdm):
                input_tensor, target = input_tensor.to(self.device), target.to(self.device)

                if self.opt.get("use_valid_subvolumes", False):
                    output = self.model(input_tensor)
                else:
                    output = self.model(input_tensor)
    ...
    def calculate_metric_and_update_statistcs(self, output, target, cur_batch_size, loss=None, mode="train"):
        ...
        for i, metric_name in enumerate(self.opt["metric_names"]):
            ...
            self.statistics_dict[mode][metric_name]["avg"] += (torch.sum(per_class_metric) / torch.sum(
    ...
    def init_statistics_dict(self):
        ...
        statistics_dict["train"]["class_count"] = {class_name: 0 for _, class_name in
                                                   self.opt["index_to_class_dict"].items()}
        statistics_dict["valid"]["class_count"] = {class_name: 0 for _, class_name in
                                                   self.opt["index_to_class_dict"].items()}
    ...
    def load(self):
        if self.opt["resume"] is not None:
            ...
            resume_state_dict = torch.load(self.opt["resume"],
            ...
            model_state_dict = torch.load(self.opt["pretrain"],
                                          map_location=lambda storage, loc: storage.cuda(self.device))
            self.model.load_state_dict(model_state_dict, strict=True)
        else:
            if self.opt["pretrain"] is not None:
                model_state_dict = torch.load(self.opt["pretrain"], map_location=self.device)
                self.model.load_state_dict(model_state_dict, strict=True)
